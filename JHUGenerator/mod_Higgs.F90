...
! Higgs decay to tau^+ tau^-   or    top anti-top		   
   SUBROUTINE EvalAmp_H_TT(pin,res,masstau)    
! Decay amplitude H --> tau^-(p1) + tau^+(p2)
! or              H --> tbar(p1)  + top(p2)
! with tau/top controlled by value of masstau
...
! R.Rontsch July 2015
      ...
      implicit none
      real(dp), intent(out) ::  res
      real(dp), intent(in) :: pin(1:4,1:2),masstau
      real(dp)             :: s12

      s12=2d0*(pin(1,1)*pin(1,2)-pin(2,1)*pin(2,2)-pin(3,1)*pin(3,2)-pin(4,1)*pin(4,2)) + 2d0*masstau**2
...
! R. Rontsch July 2015
      ...
      KL=-masstau/vev*(kappa-(0d0,1d0)*kappa_tilde)
      KR=-masstau/vev*(kappa+(0d0,1d0)*kappa_tilde)
